...
static inline void do_bmw_hash(const uint8_t *input, size_t len, uint8_t *output) {
...
void (* const extra_hashes[5])(const uint8_t *, size_t, uint8_t *) = {do_blake_hash, do_groestl_hash, do_jh_hash, do_skein_hash, do_bmw_hash};
...
inline void cryptonight_single_hash(const uint8_t *__restrict__ input, size_t size, uint8_t *__restrict__ output, cryptonight_ctx *__restrict__ ctx)
{
    ...
    keccakf(h0, 24);
    extra_hashes[ctx->state0[0] & 4](ctx->state0, 200, output);
}
...
inline void cryptonight_double_hash(const uint8_t *__restrict__ input, size_t size, uint8_t *__restrict__ output, struct cryptonight_ctx *__restrict__ ctx)
{
    ...
    extra_hashes[ctx->state0[0] & 4](ctx->state0, 200, output);
    extra_hashes[ctx->state1[0] & 4](ctx->state1, 200, output + 32);
}
